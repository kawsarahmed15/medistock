    ...
    try {
      await pool.query(
        `INSERT INTO products (id, user_id, name, category, price, cost_price, stock, expiry, sku, tax_percent, base_unit, pack_unit, conversion_factor, pack_price, pack_cost_price)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          productId, userId, name, 'General', price, costPrice, stock, expiryStr, sku, taxPercent,
          baseUnit, packUnit, conversionFactor, packPrice, packCostPrice
        ]
      );
      ...
      if (stock > 0) {
          await pool.query(
              `INSERT INTO product_history (id, product_id, user_id, action, quantity, balance, notes)
               VALUES (?, ?, ?, 'purchase', ?, ?, ?)`,
              [generateId(), productId, userId, stock, stock, `Initial import (${packQty} ${packUnit}s)`]
          );
      }
